...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
          ...
          // extend the cost with the new edge
          this.distance = copyPath.distance + extendBy.weight;
          
          this.dataSequence = new LinkedList<>();
          // copy all the vertex from the old path
          for (T i : copyPath.dataSequence) {
            this.dataSequence.add(i);
          }
          // add the new edge vertex in the sequence
    ...
    protected Path dijkstrasShortestPath(T start, T end) {    
      // if there is no vertex containing start or end
      if (this.vertices.get(start) == null || this.vertices.get(end) == null) {
        throw new NoSuchElementException("no vertex containing start or end can be found");
      }
      
      // The frontier (a minimum priority queue)
      ...
      // The shortest path to return
      Path shortestPath = null;
      
      // Get the start and end vertices from the hash table
      Vertex startVertex = vertices.get(start);
      Vertex endVertex = vertices.get(end);
      
      // if the start vertex is the end vertex
      if (startVertex.equals(endVertex)) {
        return new Path(startVertex);
      }
      
      // add the start vertex into the frontier and paths list
      Path begin = new Path(startVertex);
      frontier.add(begin);
      paths.add(begin);
      
      // check if the frontier is empty
      while(!frontier.isEmpty()) {
        // take the shortest path in the frontier
        Path shortest = frontier.poll();
        // add this shortest path's end vertex to the visited list
        visited.add(shortest.end);
        
        // obtain all the reachable edges
        LinkedList<Edge> edgesLeaving = shortest.end.edgesLeaving;
        
        for (Edge i : edgesLeaving) {
          // use to check if new extended path is already in the paths list
          Boolean isInPathList = false;
          // if the edge target has not been visited
          if (!visited.contains(i.target)) {
            // get the extended path using the extend constructor
            Path extendPath = new Path(shortest, i);
            for (Path j : paths) {
              // if the extended path is already in the paths list
              if (extendPath.end.data.equals(j.end.data)) {
                isInPathList = true;
                // if this new path is shorter than the other paths in the list, remove the old one and update paths list and frontier
                if (extendPath.distance < j.distance) {
                  paths.remove(j);
                  paths.add(extendPath);
                  frontier.add(extendPath);
                  break;
                }
              }
            }
            // if the new extended path is not in the paths list, update the paths list and frontier
            if (!isInPathList) {
              paths.add(extendPath);
              frontier.add(extendPath);
            }
      ...
      // if there is no path from start to end can be found, throw NoSuchElementException
      Boolean pathFound = false;
      for (Path i : paths) {
        if (i.end.data.equals(end)) {
          pathFound = true;
        }
      }
      if (!pathFound) {
      ...
      // Obtain the shortest path to return
      for (Path i : paths) {
        if (i.end.data.equals(end)) {
          shortestPath = i;
        }
